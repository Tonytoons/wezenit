...
################################################################################ 
    function getList($search='')
    {
        ...
        $sql2 = "SELECT COUNT(id) AS C FROM users WHERE 1 AND name LIKE '%".$search."%' OR email LIKE '%".$search."%'"; 
    ...
    function add($data)    
    { 
        $sql = $this->adapter->query("INSERT INTO `users`  
                                        (id, type, gender, active, name, email, password, phone, birth_day, address, added_date, last_update)  
                                       VALUES  
                                        ('".$data['id']."', 
                                         '".$data['type']."',
                                         '".$data['gender']."',
    ...
    { 
       ...
       if(!empty($detail['image'])){ 
         $pathDelete = 'public/img/users/'.$detail['image'];    
         @unlink($pathDelete);  
       } */ 
    ...
    {    
       $return = "false";  
       if(!empty($email)){  
            $sql = "SELECT COUNT(id) AS C FROM `users` WHERE 1 AND email ='".$email."'";  
            $statement = $this->adapter->query($sql);     
            $results = $statement->execute();
            $row = $results->current(); 
